...
//        }
    ...
    public void render(Graphics g)
    {
        ...
        checkMissile( publicCar.getX(), publicCar.getY() , player.misX , player.misY);
    }

    public void checkCollision(int x1, int y1, int x2, int y2)
    {
        if (publicCar.alive == false) return;
    ...
    public void checkMissile(int x1, int y1, int x2, int y2)
    {
        Rectangle r1 = new Rectangle(x1, y1, 75, 125);
        Rectangle r2 = new Rectangle(x2, y2, 15, 64);

        if (r1.intersects(r2)) {
         publicCar.alive = false;
